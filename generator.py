...
def main():
    ...
    for _ in range(0,num_users):
        ...
        for project, plan in zip(user['projects'], user['plan_changes']):
            ...
            print(invoices)
            support_tickets = support_ticket_generator.generate_and_insert_support_tickets(cp_connection_params, st_connection_params, project_id=project['id'], project_plan=plan['name'],
                                                                                           support_request_pattern_type=support_request_pattern_type, 
                                                                                           ticket_frequency=ticket_frequency)
